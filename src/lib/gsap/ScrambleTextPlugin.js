    ...
    for (; s < i; s++)
        (n = F.charAt(s)),
            ((n.charCodeAt(0) >= 55296 && n.charCodeAt(0) <= 56319) ||
                (F.charCodeAt(s + 1) >= 65024 &&
                    F.charCodeAt(s + 1) <= 65039)) &&
...
class E {
    ...
    grow(D) {
        for (let u = 0; u < 20; u++)
            this.sets[u] += B(D - this.length, this.chars);
        this.length = D;
    }
}
let e,
...
const g = {
    ...
    init(D, u, e) {
        ...
        return (
            ...
            (g.lengthDif =
                (B ? g.text.length : g.text.join(i).length) - g.length),
            (g.fillChar =
